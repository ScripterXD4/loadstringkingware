...
createwarning("KingWare", "KingWare V69 Loaded", 5)
createwarning("KingWare", "KingWare V69 On Top", 5)
local Players = game:GetService("Players")
...
game:GetService("UserInputService").JumpRequest:connect(function()
    ...
    Default = false,
    HoverText = "Op for travelling or going on top of stuff limit 3 or 4"
})
...
local Chat = COB("Render", {
	["Name"] = "Chat Postition V2",
	["HoverText"] = "Changes Chat Position",
	["Function"] = function(callback)
		if callback then
			game:GetService("StarterGui"):SetCore('ChatWindowPosition', UDim2.new(1.0, 0, 0.0, 146))
		else
				...
				if balloonitem then
					...
					bedwars["BalloonController"].hookBalloon = function(Self, plr, attachment, balloon)
						if tostring(plr) == lplr.Name then
							...
							spawn(function()
								wait(0.5)
								createwarning("KingWareV69", "Disabled Balllon Anticheat!", 5)
								bypassed = true
							end)
							threadidentity(2)
